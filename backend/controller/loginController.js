...
export const authenticate=async(req,res)=>{
    ...
    try {
        const waitingUser=await usermodel.findOne({email:User.email.trim()});
        if(!waitingUser)return res.status(404).json({error:"No user found with this email!"});
        const password=await bcrypt.compare(User.password,waitingUser.hashPassword)
        if(!password){
